...
void customerMenuDisplay() {
	...
	case '2':
		...
		if (search==true)
		{
			int rPrice, mPrice; bool want;
			rPrice = R.roomPriceRecognizing(ref);
			
			mPrice = 0;
			want = CR.mealNeed(ref);
			if (want == true) mPrice = MR.recognizingMealPrice(ref);
			char edit;
			cout << "                                                                           " << endl;
			cout << "  Press the given number to continue>>                                     " << endl;
			cout << "                                                                           " << endl;
			...
			switch (edit) {
			case '1':
				CR.editNoOfResidents(ref, mPrice, rPrice);
				...
				break;
			case '2':
				CR.editNoOfDates(ref, mPrice, rPrice);
