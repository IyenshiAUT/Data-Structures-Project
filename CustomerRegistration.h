...
public:
    ...
    void calculateBill(int customerRef, int mealPrice, int roomPrice)
    {
        Node* current = head;
        int reBill = 0;
        int mealP = mealPrice;
        ...
        while (current != NULL)
        {
            if (current->refNo == customerRef)
            {
                int noofDates = current->noOfDates;
                int noofResidents = current->noOfResidents;
                reBill = roomP * noofDates + mealPrice * noofDates * noofResidents;

                current->totalPrice = reBill;
    ...
    void editNoOfResidents(int customerRef, int mPrice, int rPrice) {
        Node* current = head;
        ...
        while (current != NULL) {
            ...
            if (current->refNo == customerRef) {
                if (current->paymentDone == true)
                {
                    cout << "Payment has already Done..." << endl;
                    ...
                    calculateBill(customerRef, mPrice, rPrice);
    ...
    void editNoOfDates(int customerRef, int mPrice, int rPrice) {
        Node* current = head;
